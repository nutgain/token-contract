...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xA7cc4198d53d1415E624c88d0C790f47c2f2a115;
...
contract NUTGAIN is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x68063a1a70e3a1d9B453C9546Bb73F74F113ec6b;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 50000 * 10**3 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "NUTGAIN";
    string private _symbol = "NUTG";
    ...
    uint256 private numTokensSellToAddToLiquidity = 250 * 10**3 * 10**18;
    uint256 public maxlimit = 2750 * 10**3 * 10**18;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyDevelopmentFee) external onlyOwner {
        _buyTaxFee = buyTaxFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyDevelopmentFee = buyDevelopmentFee;
    }
   
    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellDevelopmentFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(_addressFees[from].enable){
                ...
                _developmentFee = _addressFees[from]._developmentFee;
                
                // Sell
                if(to == uniswapV2Pair){
                    _taxFee = _addressFees[from]._sellTaxFee;
                    _liquidityFee = _addressFees[from]._sellLiquidityFee;
                    _developmentFee = _addressFees[from]._sellDevelopmentFee;
                }
            }
            else{
                ...
                if(_addressFees[to].enable){
                    ...
                    if(from == uniswapV2Pair){
                        ...
                        _developmentFee = _addressFees[to]._buyDevelopmentFee;
                    }
                }
            }
        }
    ...
    function setAddressFee(address _address, bool _enable, uint256 _addressTaxFee, uint256 _addressLiquidityFee, uint256 _addressDevelopmentFee) external onlyOwner {
        ...
        _addressFees[_address]._developmentFee = _addressDevelopmentFee;
    }
    
    function setBuyAddressFee(address _address, bool _enable, uint256 _addressTaxFee, uint256 _addressLiquidityFee, uint256 _addressDevelopmentFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        _addressFees[_address]._buyTaxFee = _addressTaxFee;
        _addressFees[_address]._buyLiquidityFee = _addressLiquidityFee;
        _addressFees[_address]._buyDevelopmentFee = _addressDevelopmentFee;
    }
    
    function setSellAddressFee(address _address, bool _enable, uint256 _addressTaxFee, uint256 _addressLiquidityFee, uint256 _addressDevelopmentFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        _addressFees[_address]._sellTaxFee = _addressTaxFee;
        _addressFees[_address]._sellLiquidityFee = _addressLiquidityFee;
        _addressFees[_address]._sellDevelopmentFee = _addressDevelopmentFee;
    }
